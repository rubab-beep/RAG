...
class RAGEvaluator:
    ...
    def run(
        ...
        # Choose dataset — prefer auto-generated over static
        if dataset is None:
            from eval.generate_ground_truth import load_generated_ground_truth
            auto_generated = load_generated_ground_truth()
            if auto_generated:
                dataset = auto_generated
                if verbose:
                    print(f"  Using auto-generated ground truth ({len(dataset)} questions)")
            else:
                dataset = GROUND_TRUTH
                if verbose:
                    print(f"  Using static ground truth ({len(dataset)} questions)")
        ...
        if verbose:
            print(f"\n{'='*60}")
            print(f"  RAG EVALUATION — {label.upper()}")
    ...
    def _evaluate_one(self, case: Dict, verbose: bool = False) -> Dict[str, Any]:
        ...
        faith_result = faithfulness_score(
            ...
            context_chunks = raw_chunks,
                     llm_client = getattr(self.engine, 'llm', None) if self.use_llm_faithfulness else None,
        )
        # Pass / fail logic
        if should_refuse:
            passed = not found
        ...
        return {
            ...
            "faith_detail":     faith_result.get("detail", ""),
            "sources":          sources,
            "passed":           passed,
        }
    ...
    def _save_report(self, report: Dict, label: str):
        ...
        save_data["per_question"] = [
            {k: v for k, v in pq.items() if k != "answer"}
    ...
    def _print_summary(self, report: Dict):
        ...
        if   recall >= 0.85: grade = "✅ PRODUCTION READY"
        elif recall >= 0.70: grade = "🟡 ACCEPTABLE — tune settings"
